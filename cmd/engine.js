        ...
        if (ENVIRONMENT_IS_PTHREAD) {
            ...
            class GoWasmBridge {
                constructor() {
                    ...
                    this.config = {wasmPath: './main.wasm', timeout: 10000, enableDebug: false};
                ...
                isGoMessage(data) {
                    return data && typeof data === 'object' && (data.cmd === 'goReady' || data.source === 'go-wasm');
                }
                ...
                handleGoReady(data) {
                    ...
                    self.postMessage({cmd: 'goModuleReady', availableFunctions: data.functions || [], source: 'go-wasm-bridge'});
                }
                processPendingCalls() {
                ...
                async callGoFunctions(calls) {
                    const promises = calls.map(call => this.callGoFunction(call.funcName, ...(call.args || [])));
                ...
                async callGoFunctionSafe(funcName, ...args) {
                    try {
                        ...
                        if (this.config.enableDebug) {
                            this.log('Debug information:', {funcName, args, isReady: this.isReady, availableFunctions: this.getAvailableGoFunctions()});
                        }
                        throw error;
                    }
                }
            ...
            function callMainThread(callbackName, args) {
                postMessage({cmd: 'callHandler', handler: '_spxOnMainCall', args: args ? [callbackName, ...args] : [callbackName]});
            }
            ...
            async function loadGoWasmModule() {
                ...
                try {
                    ...
                    await goBridge.initialize({wasmPath: './gdspx.wasm', timeout: 15000, enableDebug: false});
                    try {
